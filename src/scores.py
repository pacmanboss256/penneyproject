...
def create_table(data) -> pd.DataFrame:
    ...
    return pd.DataFrame(data, columns=["p1choice", "p2choice", "result"]).pivot(
        index="p1choice", columns="p2choice", values="result"
    )


class ScoreTable:
    def __init__(self, data: grid):
        self.table = create_table(data)
    ...
    def add_data(self, newData: grid) -> pd.DataFrame:
        ...
        newTable = self.table + create_table(newData)
        self.table = newTable
        return newTable
